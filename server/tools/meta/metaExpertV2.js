...
registerTool({
  ...
  description:
    ...
    "business.sampleProducts each contain id, name, price, category, imageUrl (null if the product genuinely has no image — never usable as a PRODUCT_IMAGE candidate then), permalink, and shortDescription (the product's own real description, when it has one — this is the ONLY source ever used for a PRODUCT_IMAGE ad's primary text; if it's null, the customer is asked for that text directly, never invented by you). " +
    "When build_strategy/revise_strategy resolves creative_strategy against real candidates and there's more than one eligible option, you'll get a real question back naming the real candidates by id — relay it plainly and pass the user's answer back via content_selector (position or confirmedId), never invent which one they meant. " +
    ...
    "Read the returned audienceEvidenceHint field before calling build_strategy — it tells you plainly whether this business has real data (store products/categories/pricing, or Meta campaign history) that build_strategy REQUIRES you to use to narrow gender/age instead of defaulting to ALL genders 18-65, versus a business with none yet where a generic audience is honest as-is.",
...
registerTool({
  ...
  parameters: {
    ...
    properties: {
      ...
      content_selector: {
        type: "object",
        description: "For explicit_action mode: which content to boost. For campaign mode with creative_strategy.source EXISTING_PAGE_POST/EXISTING_INSTAGRAM_POST/PRODUCT_IMAGE: which real candidate (from a prior tool result naming real candidates by id) to attach as the ad's creative. Refer to content ORDINALLY — { position: 1 } means the most recent/first item already shown — or by confirmedId, a real id already shown this conversation. Never invent a raw id. primaryTextAnswer is ONLY for a PRODUCT_IMAGE product with no real description to pull from automatically — transcribe EXACTLY what the user typed as the ad's primary text when they were asked; never write or improve it yourself.",
        properties: { position: { type: "integer" }, confirmedId: { type: "string" }, attachedMediaRef: { type: "string" }, primaryTextAnswer: { type: "string" } },
      },
...
registerTool({
  ...
  description:
    "Executes a previously built and user-APPROVED strategy — creates the real Meta Campaign, Ad Set, and (when a creative was resolved — see build_strategy/revise_strategy's content_selector) the actual Ad, all PAUSED. If the strategy's creative_strategy.source needed a real candidate picked and that's still ambiguous, this call is blocked by the same approval gate as everything else — the strategy's own unresolved_questions already asked for it; get that answer via revise_strategy first. NEVER the first tool you call for a request like 'create the best campaign' — that means build a strategy first and present it. Only call this after the user has EXPLICITLY approved the CURRENT strategy in their own words this turn ('approve', 'proceed', 'run it', 'yes, create it') — the backend enforces this and blocks the call before it reaches Meta otherwise. strategyId is optional — omit it to use the current active strategy for this conversation.",
  category: "meta_expert_v2",
